...
from(db:"test") |> range(start:-1h)`
	...
	for _, args := range []struct {
		...
		t.Run(args.caseName, func(t *testing.T) {
			if _, err := s.CreateTask(context.Background(), args.org, args.user, args.script); (err == nil) != args.noerr {
				if err != nil {
...
from(db:"test") |> range(start:-1h)`
	t.Run("happy path", func(t *testing.T) {
		...
		id, err := s.CreateTask(context.Background(), orgID, userID, fmt.Sprintf(script, 0))
		if err != nil {
			t.Fatal(err)
		}
		...
		newID, err := s.CreateTask(context.Background(), orgID, userID, fmt.Sprintf(script, 1))
		if err != nil {
			t.Fatal(err)
		}
